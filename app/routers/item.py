...
@router.get("/", response_model=list[ItemResponse],
            status_code=status.HTTP_200_OK)
async def find_by_name(db: DbDependency,
...
async def create(
        db: DbDependency, user: UserDependency, item_create: ItemCreate):
    return item_cruds.create(db, item_create, user.user_id)


@router.put("/{id}", response_model=ItemResponse,
            status_code=status.HTTP_200_OK)
async def update(db: DbDependency, item_update: ItemUpdate,
                 user: UserDependency, id: int = Path(gt=0)):
